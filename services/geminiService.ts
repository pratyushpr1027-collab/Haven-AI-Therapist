...
const callWithRetry = async (fn: (modelName: string) => Promise<any>, maxRetries = 3): Promise<any> => {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    ...
    try {
      ...
      if (is503 && attempt < maxRetries - 1) {
        console.warn(`Attempt ${attempt + 1} failed (503). Retrying in ${(attempt + 1) * 2}s with ${attempt < 1 ? primaryModel : fallbackModel}...`);
...
export const analyzeFacialExpression = async (base64ImageData: string, context: MentalHealthData): Promise<{ mood: Mood; insight: string; recommendation: Recommendation; }> => {
    ...
    try {
        const parsedResult = await callWithRetry(async (modelName: string) => {
            ...
            if (!parsed.mood || !parsed.insight || !parsed.recommendation) {
