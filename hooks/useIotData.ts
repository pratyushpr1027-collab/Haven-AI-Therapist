...
const getInitialState = (): MentalHealthData => {
  // Generate sample historical data for initial display
  const sampleBiometricHistory: BiometricHistoryEntry[] = [];
  for (let i = 20; i > 0; i--) {
    const time = new Date(Date.now() - i * 3000);
    sampleBiometricHistory.push({
      timestamp: time.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      heartRate: 65 + Math.random() * 20,
      gsr: 350 + Math.random() * 150,
    });
  }

  const sampleMoodLogs: MoodLog[] = [];
  const moods: Array<[Mood, number]> = [
    ['Calm', 4],
    ['Happy', 5],
    ['Neutral', 3],
    ['Calm', 4],
  ...
  moods.forEach((mood, idx) => {
    const time = new Date(Date.now() - (6 - idx) * 60000);
    sampleMoodLogs.push({
      id: `sample-${idx}`,
      timestamp: time.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
      mood: mood[0],
  ...
  const defaultState: MentalHealthData = {
    ...
    biometricHistory: sampleBiometricHistory,
  };
...
const useWellnessData = () => {
  ...
  const editJournalEntry = useCallback((id: string, text: string) => {
    setData(prevData => {
       if (!text.trim()) return prevData;
      const journalEntries = prevData.journalEntries.map(entry =>
        entry.id === id
          ? {
              ...entry,
              title: text.substring(0, 30) + (text.length > 30 ? '...' : ''),
  ...
  const deleteJournalEntry = useCallback((id: string) => {
    setData(prevData => ({
      ...prevData,
      journalEntries: prevData.journalEntries.filter(entry => entry.id !== id),
    }));
  }, []);
  ...
  return { data, addMoodLog, addActivityLog, addJournalEntry, editJournalEntry, deleteJournalEntry, addGoal, toggleGoal };
};
